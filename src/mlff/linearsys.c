...
void copy_descriptors(DescriptorObj *desc_str_MLFF, DescriptorObj *desc_str){
	...
	for (int i = 0; i < desc_str->natom_domain; i++){
		memcpy(desc_str_MLFF->X2[i], desc_str->X2[i], desc_str->size_X2*sizeof(double));
		...
		for (int j = 0; j < 1+uniq_natms; j++){
			memcpy(desc_str_MLFF->dX2_dX[i][j], desc_str->dX2_dX[i][j], desc_str->size_X2*sizeof(double));
			memcpy(desc_str_MLFF->dX2_dY[i][j], desc_str->dX2_dY[i][j], desc_str->size_X2*sizeof(double));
			memcpy(desc_str_MLFF->dX2_dZ[i][j], desc_str->dX2_dZ[i][j], desc_str->size_X2*sizeof(double));
		}
		for (int j = 0; j < 6; j++){
			memcpy(desc_str_MLFF->dX2_dF[i][j], desc_str->dX2_dF[i][j], desc_str->size_X2*sizeof(double));
		}
	}
...
void add_firstMD(DescriptorObj *desc_str, NeighList *nlist, MLFF_Obj *mlff_str, double E, double *F, double *stress_sparc) {
	...
	int size_X2, size_X3;
	double beta_2, beta_3;
	size_X2 = desc_str->size_X2;
	beta_2 = desc_str->beta_2;
	beta_3 = desc_str->beta_3;
	...
	X2_gathered = (double *) malloc(sizeof(double)*size_X2*desc_str->natom);
	...
	for (int i=0; i < mlff_str->natom_domain; i++){
		for (int j=0; j < size_X2; j++){
			X2_local[i*size_X2+j] = desc_str->X2[i][j];
		}
	...
	// if (rank == 0) {
	// 	fprintf(mlff_str->fp_mlff, "X2\n");
	// 	for (int i=0; i < natom; i++){
	// 		for (int j=0; j < size_X2; j++){
	// 			fprintf(mlff_str->fp_mlff, "%lf ", X2_gathered_2D[i][j]);
	// 		}
	// 		fprintf(mlff_str->fp_mlff, "\n");
	// 	}
	// 	fprintf(mlff_str->fp_mlff, "X3\n");
	// 	for (int i=0; i < natom; i++){
	// 		for (int j=0; j < size_X3; j++){
	// 			fprintf(mlff_str->fp_mlff, "%lf ", X3_gathered_2D[i][j]);
	// 		}
	// 		fprintf(mlff_str->fp_mlff, "\n");
	// 	}
	// }
	...
	for (int i=0; i <nelem; i++){
		...
		mlff_CUR_sparsify(kernel_typ, &X2_gathered_2D[cum_natm_elem[i]], &X3_gathered_2D[cum_natm_elem[i]],
				 desc_str->natom_elem[i], size_X2, size_X3, beta_2, beta_3, xi_3, &highrank_ID_descriptors[i], N_low_min);
	}
	...
	if (rank == 0) fprintf(mlff_str->fp_mlff, "natm_train_total: %d\n", mlff_str->natm_train_total);
	if (rank==0){
	...
	for (int i=0; i < desc_str->natom_domain; i++){
		...
		for (int iatm_el_tr = 0; iatm_el_tr < (highrank_ID_descriptors[el_type]).len; iatm_el_tr++){
			...
			K_train_local[col_idx] += temp * mlff_kernel(kernel_typ, desc_str->X2, desc_str->X3,
														 mlff_str->X2_traindataset, mlff_str->X3_traindataset, i, col_idx,
	...
	for (int i=0; i <desc_str->natom_domain; i++){
		...
		for (int iatm_el_tr = 0; iatm_el_tr < (highrank_ID_descriptors[el_type]).len; iatm_el_tr++){
			...
			K_train_local[row_idx*mlff_str->n_cols + col_idx] +=  
				der_mlff_kernel(kernel_typ, desc_str->dX2_dX, desc_str->dX3_dX,
				desc_str->X2, desc_str->X3,
				mlff_str->X2_traindataset, mlff_str->X3_traindataset, i, col_idx, 0,
				beta_2, beta_3, xi_3, size_X2, size_X3);

			K_train_local[(row_idx+1)*mlff_str->n_cols + col_idx] +=  
				der_mlff_kernel(kernel_typ, desc_str->dX2_dY, desc_str->dX3_dY,
				desc_str->X2, desc_str->X3,
				mlff_str->X2_traindataset, mlff_str->X3_traindataset, i, col_idx, 0,
				beta_2, beta_3, xi_3, size_X2, size_X3);

			K_train_local[(row_idx+2)*mlff_str->n_cols + col_idx] +=  
				der_mlff_kernel(kernel_typ, desc_str->dX2_dZ, desc_str->dX3_dZ,
				desc_str->X2, desc_str->X3,
				mlff_str->X2_traindataset, mlff_str->X3_traindataset, i, col_idx, 0,
				beta_2, beta_3, xi_3, size_X2, size_X3);
			...
			for (int neighs =0; neighs < desc_str->unique_Nneighbors[i]; neighs++){
				...
				K_train_local[row_idx*mlff_str->n_cols + col_idx] +=  
					der_mlff_kernel(kernel_typ, desc_str->dX2_dX, desc_str->dX3_dX,
					desc_str->X2, desc_str->X3,
					mlff_str->X2_traindataset, mlff_str->X3_traindataset, i, col_idx, 1+neighs,
					beta_2, beta_3, xi_3, size_X2, size_X3);
		...
		for (int iatm_el_tr = 0; iatm_el_tr < (highrank_ID_descriptors[el_type]).len; iatm_el_tr++){
			...
			for (int istress = 0; istress < mlff_str->stress_len; istress++){
				...
				K_train_local[(row_idx)*mlff_str->n_cols + col_idx] +=  (1.0/volume)*
						der_mlff_kernel(kernel_typ, desc_str->dX2_dF, desc_str->dX3_dF,
						desc_str->X2, desc_str->X3,
						mlff_str->X2_traindataset, mlff_str->X3_traindataset, i, col_idx, istress,
						beta_2, beta_3, xi_3, size_X2, size_X3);
			}
		}
	}
...
void add_newstr_rows(DescriptorObj *desc_str, NeighList *nlist, MLFF_Obj *mlff_str, double E, double *F, double *stress_sparc) {
	...
	int  natom = desc_str->natom;
	int nelem = desc_str->nelem	;
	int size_X2 = desc_str->size_X2;
	int size_X3 = desc_str->size_X3;
	double beta_2 = desc_str->beta_2;
	double beta_3 = desc_str->beta_3;
	double xi_3 = desc_str->xi_3;
...
*/
 ...
 void add_newtrain_cols(double *X2, double *X3, int elem_typ, MLFF_Obj *mlff_str){
		...
		for (int i=0; i <(mlff_str->descriptor_strdataset+istr)->natom_domain; i++){
			...
			if (el_type==elem_typ){
				k_local[row_idx] +=  temp*
					mlff_kernel(kernel_typ, (mlff_str->descriptor_strdataset+istr)->X2, (mlff_str->descriptor_strdataset+istr)->X3,
					mlff_str->X2_traindataset, mlff_str->X3_traindataset, i, mlff_str->n_cols, beta_2, beta_3, xi_3, size_X2, size_X3);
			}
		}
	...
	int row_index_F;
	for (int istr = 0; istr < mlff_str->n_str; istr++){
		for (int iatm_str = 0; iatm_str < (mlff_str->descriptor_strdataset+istr)->natom_domain; iatm_str++){
			el_type = (mlff_str->descriptor_strdataset+istr)->el_idx_domain[iatm_str];
			if (el_type==elem_typ){
				atom_idx = (mlff_str->descriptor_strdataset+istr)->atom_idx_domain[iatm_str];
				...
				k_local[row_index_F] +=  
					der_mlff_kernel(kernel_typ, (mlff_str->descriptor_strdataset+istr)->dX2_dX, (mlff_str->descriptor_strdataset+istr)->dX3_dX,
					(mlff_str->descriptor_strdataset+istr)->X2, (mlff_str->descriptor_strdataset+istr)->X3,
					mlff_str->X2_traindataset, mlff_str->X3_traindataset, iatm_str, mlff_str->n_cols, 0, beta_2, beta_3, xi_3, size_X2, size_X3);
				...
				k_local[row_index_F+1] +=  
					der_mlff_kernel(kernel_typ, (mlff_str->descriptor_strdataset+istr)->dX2_dY, (mlff_str->descriptor_strdataset+istr)->dX3_dY,
					(mlff_str->descriptor_strdataset+istr)->X2, (mlff_str->descriptor_strdataset+istr)->X3,
					mlff_str->X2_traindataset, mlff_str->X3_traindataset, iatm_str, mlff_str->n_cols, 0, beta_2, beta_3, xi_3, size_X2, size_X3);
				...
				k_local[row_index_F+2] +=  
					der_mlff_kernel(kernel_typ, (mlff_str->descriptor_strdataset+istr)->dX2_dZ, (mlff_str->descriptor_strdataset+istr)->dX3_dZ,
					(mlff_str->descriptor_strdataset+istr)->X2, (mlff_str->descriptor_strdataset+istr)->X3,
					mlff_str->X2_traindataset, mlff_str->X3_traindataset, iatm_str, mlff_str->n_cols, 0, beta_2, beta_3, xi_3, size_X2, size_X3);
				for (int neighs = 0; neighs < (mlff_str->descriptor_strdataset+istr)->unique_Nneighbors[iatm_str]; neighs++){
					row_index_F = row_idx + 3*(mlff_str->descriptor_strdataset+istr)->unique_neighborList[iatm_str].array[neighs]+1;
					k_local[row_index_F] +=  
					der_mlff_kernel(kernel_typ, (mlff_str->descriptor_strdataset+istr)->dX2_dX, (mlff_str->descriptor_strdataset+istr)->dX3_dX,
						(mlff_str->descriptor_strdataset+istr)->X2, (mlff_str->descriptor_strdataset+istr)->X3,
						mlff_str->X2_traindataset, mlff_str->X3_traindataset, iatm_str, mlff_str->n_cols, 1+neighs, beta_2, beta_3, xi_3, size_X2, size_X3);
		...
		for (int iatm_str = 0; iatm_str < (mlff_str->descriptor_strdataset+istr)->natom_domain; iatm_str++){
			...
			if (el_type==elem_typ){
				for (int istress = 0; istress < mlff_str->stress_len; istress++){
					...
					der_mlff_kernel(kernel_typ, (mlff_str->descriptor_strdataset+istr)->dX2_dF, (mlff_str->descriptor_strdataset+istr)->dX3_dF,
						(mlff_str->descriptor_strdataset+istr)->X2, (mlff_str->descriptor_strdataset+istr)->X3,
						mlff_str->X2_traindataset, mlff_str->X3_traindataset, iatm_str, mlff_str->n_cols, istress, beta_2, beta_3, xi_3, size_X2, size_X3);				
				}
			}
		}
		row_idx += 1+3*(mlff_str->descriptor_strdataset+istr)->natom+mlff_str->stress_len;
