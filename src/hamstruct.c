 ...
 * to apply the Hamiltonian as an operator.
 * In this case, it applies the Laplacian, Veff, and NonLocal as separate steps
 **/
#ifdef __cplusplus
extern "C" {
#endif

void Our_Hamiltonian(const void* ham_struct, const Psi_Info* const psi_in,
                     Psi_Info* psi_out, double c) {
  ...
  if (PCE_Internal_debug_level() <= DEBUG_TRACE) {
    double loc_psi =
        PCE_Internal_sum_abs(psi_in->data, ohs->hd->local_num_fd,
                             ohs->hd->local_num_cols, ohs->compute_device);
    double glob_psi = 0;
  ...
  PCE_Laplacian(ohs->hd, ohs->fd_info, psi_in, psi_out, ohs->laplacian_scaling,
                ohs->communication_device, ohs->compute_device,
                ohs->comm);  // Nabla
#if USE_GPU
  if (ohs->compute_device == DEVICE_TYPE_DEVICE) {
    gpuErrchk(cudaPeekAtLastError());
  ...
  if (PCE_Internal_debug_level() <= DEBUG_TRACE) {
    ...
    double loc_psi =
        PCE_Internal_sum_abs(psi_out->data, ohs->hd->local_num_fd,
                             ohs->hd->local_num_cols, ohs->compute_device);
    double glob_psi = 0;
  ...
  //        PCE_Internal_sum_abs(psi_out->data,
  //        ohs->hd->local_num_fd,ohs->hd->local_num_cols,
  //        ohs->compute_device));
  //  }
  //#endif
  ...
  if (fabs(ohs->veff_scaling) > 1e-12) {
    PCE_Veff_Apply_Inplace(
        ohs->veff_info, ohs->hd->local_num_cols, ohs->hd->local_num_fd, psi_in,
        psi_out, ohs->veff_scaling, ohs->compute_device);  // V_XC, V_H
  }
#if USE_GPU
  if (ohs->compute_device == DEVICE_TYPE_DEVICE) {
    gpuErrchk(cudaPeekAtLastError());
  ...
  if (c != 0) {
    ...
    PCE_Internal_Shift(psi_in, psi_out,
                       ohs->hd->local_num_cols * ohs->hd->local_num_fd, -c);
#if USE_GPU
    if (ohs->compute_device == DEVICE_TYPE_DEVICE) {
      gpuErrchk(cudaPeekAtLastError());
      gpuErrchk(cudaDeviceSynchronize());
  ...
  if (ohs->do_nonlocal) {
    PCE_NonLocal_Apply(ohs->nonlocal_info, ohs->hd, psi_in, psi_out,
                       ohs->communication_device, ohs->compute_device,
                       ohs->comm);  // V_ion
  }
...
#if DEBUG
  if (PCE_Internal_debug_level() <= DEBUG_TRACE) {
    double post_nonlocal_loc =
        PCE_Internal_sum_abs(psi_out->data, ohs->hd->local_num_fd,
                             ohs->hd->local_num_cols, ohs->compute_device);
    double glob_nonlocal = 0;
    MPI_Allreduce(&post_nonlocal_loc, &glob_nonlocal, 1, MPI_DOUBLE, MPI_SUM,
                  MPI_COMM_WORLD);
    if (rank == 0) {
